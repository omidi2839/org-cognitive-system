...
export default async function handler(req,res){
 ...
 try{
  ...
  const rows=(db.normalizedDocuments||[]).filter(x=>(x.documentRef||x.documentId)===documentId&&x.organizationId===org&&x.role!=='attachment'&&x.status!=='superseded');
  const n=rows.find(x=>x.id===doc.normalizedRef)||rows.sort((a,b)=>(b.sourceVersion||1)-(a.sourceVersion||1))[0];
