...
const service=new CognitiveService(repository,new MockAIGateway(),createArtifactStorage());
...
export default async function handler(req,res){
  try{
    ...
    if(path==='/api/v1/health'&&req.method==='GET') return send(res,200,{status:'ok',version:'0.7.6.2',environment:process.env.VERCEL_ENV||'local',persistence:repositoryMode(),storage:storageMode()});
