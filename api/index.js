...
const headersToObject=req=>Object.fromEntries(
...
async function bootApplication(){
  ...
  appPromise=(async()=>{
    ...
    const getCognitiveService=()=>{
      if(!cognitivePromise){
        cognitivePromise=import('../src/application/cognitiveDocumentUnderstanding077.js')
          .then(({CognitiveDocumentUnderstandingService})=>
            new CognitiveDocumentUnderstandingService(repository,aiGateway,artifactStorage)
          );
      }
      return cognitivePromise;
    };
...
async function coldBootDiagnostic(){
  ...
  const step=async(name,fn)=>{
    ...
    try{
      ...
      report.steps.push({
        ...
        error:{name:err?.name||null,code:err?.code||null,message:err?.message||String(err)}
      });
  ...
  const kd=await step('service.knowledgeDocuments(upstream)',()=>service.knowledgeDocuments(actor,'upstream'));
  report.knowledgeSummary=kd?.summary||null;
  report.ok=true;
  return report;
}
...
export default async function handler(req,res){
  ...
  if(path==='/api/v1/diagnostics/cold-boot'&&req.method==='GET'){
    try{
      const report=await coldBootDiagnostic();
  ...
  try{
    ...
    if(path==='/api/v1/health/ready'&&req.method==='GET'){
      ...
      const st=service.storage;
      const storageOk=typeof st.health==='function'?await st.health():true;
      const durableRequired=String(process.env.REQUIRE_DURABLE_SERVICES||'').toLowerCase()==='true';
      const durableOk=!durableRequired||(repositoryMode()==='postgres'&&storageMode()==='vercel-blob');
      const ok=dbOk&&storageOk&&durableOk;
      return send(res,ok?200:503,{status:ok?'ready':'not_ready',persistence:repositoryMode(),storage:storageMode(),durableRequired,checks:{database:dbOk,storage:storageOk,durable:durableOk}});
    }
    ...
    if(path.startsWith('/api/v1/sources/')&&req.method==='GET'){
      const ref=decodeURIComponent(path.slice('/api/v1/sources/'.length));
      return send(res,200,await service.resolveSourceReference(actor,ref));
    }
    ...
    const pm=decodedPath.match(/^\/api\/v1\/documents\/(DOC:[^/]+)\/process$/);
    if(pm&&req.method==='POST') return send(res,200,await service.processDocument(actor,pm[1]));
    const tm=decodedPath.match(/^\/api\/v1\/documents\/(DOC:[^/]+)\/trace$/);
    if(tm&&req.method==='GET') return send(res,200,await service.trace(actor,tm[1]));
    const cm=decodedPath.match(/^\/api\/v1\/candidates\/(CAND:[^/]+)\/review$/);
    if(cm&&req.method==='POST') return send(res,200,await service.reviewCandidate(actor,cm[1],req.body||{}));

    return send(res,404,{code:'NOT_FOUND',message:'مسیر پیدا نشد.'});
  }catch(e){
    console.error(e);
    return send(res,e.code?.includes('NOT_FOUND')?404:e.code?.includes('DENIED')?403:400,{
      code:e.code||'INTERNAL_ERROR',
      message:e.message||'خطای داخلی'
    });
  }
}
