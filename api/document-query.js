...
export default async function handler(req,res){
 ...
 try{
  ...
  const matchedIssuer=actualIssuers.sort((a,b)=>String(b).length-String(a).length).find(x=>nq.includes(norm(x)))||null;
  ...
  const known=[matchedTopic,matchedIssuer,type,'اصلاحیه','الحاقیه','استفسار','ملغی','لغو','معتبر'];
  ...
  if(matchedIssuer)docs=docs.filter(x=>norm(field(x.d,x.m,'issuer')).includes(norm(matchedIssuer)));
  ...
  if(mode==='qa'||/(عبارت|صحبت|آمده|ذکر|درباره|در مورد)/.test(nq)){
    ...
    return send(res,200,{answer:{summary,scopeLabel:'بانک اسناد',evidence:ev},items:docs.slice(0,30).map(itemOf),parsed:{topic:matchedTopic,issuer:matchedIssuer,documentNumber:dno||null,meetingNumber:mno||null,years:yrs,relationType:relType}});
  }

  let items=[];for(const x of docs){const hay=norm([x.d.title,field(x.d,x.m,'documentType'),field(x.d,x.m,'subjectCategory'),field(x.d,x.m,'subjectArea'),field(x.d,x.m,'issuer'),x.text].join(' '));if(terms.length&&!terms.some(t=>hay.includes(t)))continue;const ss=sentences(x.text).filter(st=>terms.some(t=>norm(st).includes(t)));items.push({...itemOf(x),excerpt:ss[0]||String(x.text).slice(0,360)})}
  return send(res,200,{items:items.slice(0,50),parsed:{topic:matchedTopic,issuer:matchedIssuer,documentNumber:dno||null,meetingNumber:mno||null,years:yrs,relationType:relType}});
