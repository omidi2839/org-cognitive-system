(()=>{
window.__DOCUMENT_MEETING_COMMAND_BUILD__='0.9.5.7.2';
...
function k957SimplifyRegisterStatus(){
 ...
 const apply=()=>{
   ...
   if(/در حال ثبت|در حال/i.test(t)){st.textContent='در حال ثبت سند…';st.classList.add('k957working');return}
   if(/ثبت شد|با موفقیت|شناسه:/i.test(t)){st.textContent='سند با موفقیت ثبت شد.';st.classList.add('k957ok');return}
   if(/خطا|مشکل|ناموفق|الزامی|پیدا نشد|نیازمند/i.test(t)){st.textContent='در ثبت سند مشکلی به وجود آمد.';st.classList.add('k957err');}
 };
 new MutationObserver(apply).observe(st,{subtree:true,childList:true,characterData:true});
 apply();
}
...
function enhance(){document.querySelectorAll('#k76form').forEach(f=>{enhanceUploadForm(f);k953EnhanceMetadata(f);k955ComposeForm(f);k957SimplifyRegisterStatus()});const modal=document.getElementById('k91docmodal');if(modal){collapseRelations(modal.querySelector('.k944relations'));addPopupTools(modal);k955RenderStructuredDoc()}}
