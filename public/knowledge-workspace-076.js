...
function renderUnderstanding(id,a,versions){
 ...
 <div class="k78quality"><b>کیفیت ورود و استخراج</b><span>${a.extractionQuality?Math.round(a.extractionQuality.score*100)+'٪ · '+esc(a.extractionQuality.message):'—'}</span></div><div class="k77cols"><div class="k77box"><h4>مفاهیم منتخب از گزاره‌ها</h4>${(a.concepts||[]).map(c=>`<div class="k77concept"><b>${esc(c.label)}</b><small>${c.organizationalMeaning?'معنای سازمانی: '+esc(c.organizationalMeaning):'معنا هنوز Candidate است'} · ${Math.round(c.confidence*100)}٪</small></div>`).join('')||'<small>مفهومی کشف نشد.</small>'}</div>
 ...
 <div class="k77box questions"><h4>پرسش‌های شناختی برای تکمیل فهم</h4>${(a.questions||[]).map(q=>q.status==='answered'?`<div class="k77answered"><b>${q.semanticContext?.highlightConcepts?.length?`<div class="k82concepts">${q.semanticContext.highlightConcepts.map(c=>`<span title="مفهوم/عنصر مهم کشف‌شده از همین گزاره">${esc(c)}</span>`).join('')}</div>`:''}${q.semanticContext?.interpretation?`<div class="k82interpret"><b>برداشت اولیه سامانه:</b> ${esc(q.semanticContext.interpretation)}</div>`:''}<div class="k82question">${esc(q.question)}</div>${q.semanticContext?.proposedRelations?.length?`<div class="k82relations"><b>نسبت‌های پیشنهادی:</b>${q.semanticContext.proposedRelations.map(r=>`<span class="k82relation" title="نسبت مفهومی پیشنهادی">${esc(r)}</span>`).join('')}</div>`:''}</b><p>پاسخ: ${esc(q.answer)}</p></div>`:`<form data-cq="${esc(q.id)}" data-doc="${esc(id)}"><b>${q.semanticContext?.highlightConcepts?.length?`<div class="k82concepts">${q.semanticContext.highlightConcepts.map(c=>`<span title="مفهوم/عنصر مهم کشف‌شده از همین گزاره">${esc(c)}</span>`).join('')}</div>`:''}${q.semanticContext?.interpretation?`<div class="k82interpret"><b>برداشت اولیه سامانه:</b> ${esc(q.semanticContext.interpretation)}</div>`:''}<div class="k82question">${esc(q.question)}</div>${q.semanticContext?.proposedRelations?.length?`<div class="k82relations"><b>نسبت‌های پیشنهادی:</b>${q.semanticContext.proposedRelations.map(r=>`<span class="k82relation" title="نسبت مفهومی پیشنهادی">${esc(r)}</span>`).join('')}</div>`:''}</b><small>${esc(q.reason)}</small>${q.evidence?`<blockquote><strong class="k82evidenceLabel">شاهد از سند:</strong> ${esc(q.evidence)}</blockquote>`:''}<textarea required placeholder="پاسخ شما برای اصلاح معنای سازمانی…"></textarea><button>ثبت پاسخ و اصلاح مدل فهم</button></form>`).join('')||'<small>پرسش بازی وجود ندارد.</small>'}</div>
