...
function renderQuestion(q,id,index){
 ...
 if(q.status==='answered')return `<div class="k831question answered"><small>پرسش ${index+1}</small>${concepts.length?`<div class="k82concepts">${concepts.map(c=>`<span title="مفهوم/عنصر مهم همین گزاره">${esc(c)}</span>`).join('')}</div>`:''}<div class="k82question">${esc(q.question)}</div>${rels.length?`<div class="k82relations"><b>نسبت‌های پیشنهادی:</b>${rels.map(x=>`<span class="k82relation">${esc(x)}</span>`).join('')}</div>`:''}<p>پاسخ: ${esc(q.answer)}</p></div>`;
 return `<form class="k831question" data-cq="${esc(q.id)}" data-doc="${esc(id)}"><small>پرسش ${index+1}</small>${concepts.length?`<div class="k82concepts">${concepts.map(c=>`<span title="مفهوم/عنصر مهم همین گزاره">${esc(c)}</span>`).join('')}</div>`:''}${q.semanticContext?.interpretation?`<div class="k82interpret"><b>برداشت اولیه سامانه:</b> ${esc(q.semanticContext.interpretation)}</div>`:''}<div class="k82question">${esc(q.question)}</div>${rels.length?`<div class="k82relations"><b>نسبت‌های پیشنهادی:</b>${rels.map(x=>`<span class="k82relation">${esc(x)}</span>`).join('')}</div>`:''}<small>${esc(q.reason)}</small><textarea required placeholder="پاسخ شما برای اصلاح معنای سازمانی…"></textarea><button>ثبت پاسخ و اصلاح مدل فهم</button></form>`;
}
