(()=>{
window.__DOCUMENT_MEETING_COMMAND_BUILD__='0.9.5.7.3';
 ...
 if(subject){
   ...
   async function analyze(){
     ...
     try{
       ...
       status.innerHTML=d.recommended?.length?`پیشنهاد سامانه: ${d.recommended.slice(0,4).map((x,i)=>`<b>${i===0?'★ ':''}${esc(x.label)}</b>${x.source==='title'?'<small class="k958source">از عنوان سند</small>':x.source==='heading'?'<small class="k958source">از سوتیتر</small>':''}`).join('، ')}`:'موضوع غالب با اطمینان کافی تشخیص داده نشد؛ از فهرست انتخاب کنید.';
