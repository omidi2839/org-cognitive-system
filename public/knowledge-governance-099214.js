(()=>{
window.__KNOWLEDGE_GOVERNANCE_BUILD__='0.9.9.2.14';
...
/*
 * Permanent counter contract:
 * - Every active capability card may expose a counter.
 * - Counters are derived from authoritative APIs, never from DOM text.
 * - Any mutation dispatches `sina:data-changed` and forces a fresh read.
 * - Composite cards (e.g. macro knowledge) can display more than one metric.
 * - Future modules only need to add a metric here; card rendering stays generic.
 */
function k9931CounterLabel(name,m){
 const v=m?.[name];
 if(v==null)return null;
 if(typeof v==='object'&&!Array.isArray(v)){
   const parts=[];
   if(Number.isFinite(Number(v.concepts)))parts.push(`${fa(v.concepts)} مفهوم`);
   if(Number.isFinite(Number(v.knowledge)))parts.push(`${fa(v.knowledge)} دانش`);
   if(Number.isFinite(Number(v.items)))parts.push(`${fa(v.items)} مورد`);
   return parts.join(' · ');
 }
 const unit=(name==='دانش کلان')?'مفهوم':(name==='تحلیل اسناد'?'سند':'سند');
 return `${fa(v)} ${unit}`;
}
function k9931ApplyCounters(){
 const m=k9931CounterCache;if(!m)return;
 document.querySelectorAll('.capability-card,.k9931-static-document-card,[data-count-key]').forEach(c=>{
   const n=c.dataset.countKey||c.dataset.k9931Label||c.dataset.capability||c.querySelector('b')?.textContent?.trim()||c.querySelector('h4')?.textContent?.trim();
   const next=k9931CounterLabel(n,m);
   if(next==null)return;
   let b=c.querySelector('.k980count');
   if(!b){b=document.createElement('span');b.className='k980count';c.appendChild(b)}
...
async function counters(force=false){
 ...
 if(!force&&k9931CounterCache&&(now-k9931CounterFetchedAt)<8000){
   k9931ApplyCounters();return k9931CounterCache
 }
 k9931CounterInFlight=(async()=>{
  try{
   const qs=`?snippetLimit=1&_counter=${Date.now()}`;
   const fresh={cache:'no-store',headers:{'cache-control':'no-cache','pragma':'no-cache'}};
   const [bankR,analysisR,macroR]=await Promise.allSettled([
    api('/api/v1/knowledge/document-bank'+qs,fresh),
    api('/api/v1/knowledge/collaborative-analysis',fresh),
    api('/api/v1/knowledge/macro-knowledge',fresh)
   ]);

   // IMPORTANT: card counters are derived from the exact same active document set
   // that powers "بانک اسناد". This prevents prepared/deleted/internal rows from
   // inflating the dashboard counters.
   const bankItems=bankR.status==='fulfilled'?(bankR.value.items||[]):[];
   const bankTotal=bankR.status==='fulfilled'
      ?Number(bankR.value.summary?.visible ?? bankItems.length)
      :Number(k9931CounterCache?.['بانک اسناد']||0);

   const upstream=bankR.status==='fulfilled'
      ?bankItems.filter(x=>x.documentClass==='upstream').length
      :Number(k9931CounterCache?.['اسناد بالادستی']||0);
   const general=bankR.status==='fulfilled'
      ?bankItems.filter(x=>x.documentClass==='general').length
      :Number(k9931CounterCache?.['اسناد عمومی']||0);

   const analyzed=analysisR.status==='fulfilled'
      ?(analysisR.value.items||[]).filter(x=>x.case).length
      :Number(k9931CounterCache?.['تحلیل اسناد']||0);

   const macro=macroR.status==='fulfilled'?macroR.value:null;
   const canonical=macro?Number(macro.summary?.stabilizedConcepts ?? macro.summary?.canonicalConcepts ?? 0)
      :Number(k9931CounterCache?.['دانش کلان']?.concepts||0);
   const acceptedKnowledge=macro?Number(macro.summary?.acceptedKnowledge||0)
      :Number(k9931CounterCache?.['دانش کلان']?.knowledge||0);

   k9931CounterCache={
    'اسناد بالادستی':upstream,
    'اسناد عمومی':general,
    'بانک اسناد':bankTotal,
    'تحلیل اسناد':analyzed,
    'دانش کلان':{concepts:canonical,knowledge:acceptedKnowledge}
   };
   k9931CounterFetchedAt=Date.now();
   k9931ApplyCounters();
   return k9931CounterCache;
  }finally{k9931CounterInFlight=null}
 })();
 return k9931CounterInFlight;
}
window.__SINA_REFRESH_COUNTERS__=()=>counters(true);
window.addEventListener('sina:data-changed',()=>{k9931CounterFetchedAt=0;counters(true)});
window.addEventListener('focus',()=>{if(Date.now()-k9931CounterFetchedAt>10000)counters(true)});
...
async function openCase(id){
 ...
 const formHtml=!approved?`<form id="k983annotationform">
      ...
      <section class="k9929-aiassist"><header><b>${stage1?'دستیار تحلیل خبره':stage2?'دستیار نقد خبره ارشد':'دستیار جمع‌بندی'}</b><small>پیشنهاد هوش مصنوعی جایگزین نظر خبره نیست و قبل از ثبت قابل ویرایش است.</small></header><button type="button" data-k9929-ai-assist>پیشنهاد هوش مصنوعی</button><small data-k9929-ai-status></small></section>
 ...
 if(form){
  ...
  const aiBtn=form.querySelector('[data-k9929-ai-assist]'),aiStatus=form.querySelector('[data-k9929-ai-status]');
  if(aiBtn)aiBtn.onclick=async()=>{
    const concept=form.querySelector('[data-concept]'),evidence=form.querySelector('[data-evidence]'),analysisBox=form.querySelector('textarea[name="analysis"]');
    if(!stage1&&!String(concept?.value||'').trim()){aiStatus.textContent='ابتدا یک مفهوم از فهرست انتخاب کنید.';return}
    aiBtn.disabled=true;aiStatus.textContent='در حال تحلیل هوشمند بر اساس متن سند و سوابق خبرگان…';
    try{
      const r=await api('/api/v1/knowledge/collaborative-analysis',{method:'PATCH',body:JSON.stringify({caseId:c.id,action:'ai_assist',concept:String(concept?.value||''),evidence:String(evidence?.value||'')})});
      const s=r.suggestion||{};
      if(stage1&&s.concept&&!concept.value)concept.value=s.concept;
      if(s.evidence&&!evidence.value)evidence.value=s.evidence;
      if(s.analysis)analysisBox.value=s.analysis;
      aiStatus.textContent=`پیشنهاد آماده شد${s.confidence?` · اطمینان تقریبی ${fa(Math.round(s.confidence*100))}٪`:''}${s.cautions?.length?` · ${s.cautions.join('؛ ')}`:''}`;
      analysisBox.focus();
    }catch(err){aiStatus.textContent=err.message||'تحلیل هوشمند انجام نشد.'}
  ...
  record.onclick=async()=>{
   try{
    ...
    recorder.start();record.disabled=true;stop.disabled=false;status.textContent='در حال ضبط… حداکثر حدود ۹۰ ثانیه.';
...
function k9913CanonicalCard(c){
 return `<article class="k9913canonical"><header><div><small>${esc(c.topic||'مفهوم سازمانی')}</small><h4>${esc(c.title)}</h4></div><span>${c.stabilized?'تثبیت‌شده':'در انتظار تعیین تکلیف'}</span></header><p class="k9913long-fa" style="direction:rtl;text-align:justify;text-align-last:right;max-height:190px;overflow-y:auto;overflow-x:hidden;padding-left:6px;line-height:2">${esc(c.definition||'')}</p><footer><small>${fa(c.sourceCount||0)} منشأ معتبر</small></footer></article>`;
}
function k9913KnowledgeCard(k){
 ...
 return `<article class="k9913knowledge-card ${candidate?'candidate':'accepted'}">
  ...
  <p class="k9913long-fa" style="direction:rtl;text-align:justify;text-align-last:right;max-height:230px;overflow-y:auto;overflow-x:hidden;padding-left:6px;line-height:2">${esc(k.synthesis||'')}</p>
  <div class="k9913concept-chips">${(k.conceptTitles||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div>
  ${candidate?`<footer><button class="k9929-ai-knowledge" data-k9929-ai-knowledge="${esc(k.id)}">تولید دانش با هوش مصنوعی</button><button data-k9913-accept-knowledge="${esc(k.id)}">تثبیت به‌عنوان دانش کلان</button></footer>`:''}
 </article>`;
}
function k9913BuildMap(stabilized,relations,knowledge){
 ...
 for(const c of stabilized){
   ...
   nodes.push(`<g class="k9913map-node concept" data-k9913-node="${esc(c.id)}" role="button"><circle cx="${p.x}" cy="${p.y}" r="${rad}"/><text x="${p.x}" y="${p.y+4}" text-anchor="middle">${esc((c.title||'').slice(0,18))}</text><title>${esc(c.title)} · مرکزیت دانشی ${fa(Math.round((c.knowledgeCentrality||0)*100))}٪</title></g>`);
 }
 for(const k of ks){
   const p=pos[k.id],rad=46+Math.min(18,(k.conceptRefs?.length||2)*3);
   nodes.push(`<g class="k9913map-node knowledge ${k.status==='system_candidate'?'candidate':''}" data-k9913-node="${esc(k.id)}" role="button"><circle cx="${p.x}" cy="${p.y}" r="${rad}"/><text x="${p.x}" y="${p.y-2}" text-anchor="middle">دانش</text><text x="${p.x}" y="${p.y+14}" text-anchor="middle">${esc((k.topic||'ترکیبی').slice(0,15))}</text><title>${esc(k.title)}</title></g>`);
 }
...
async function openMacroKnowledgeV13(){
 ...
 try{
  ...
  b.querySelectorAll('[data-k9913-node]').forEach(node=>node.onclick=()=>{
    ...
    if(obj.conceptRefs){
      box.innerHTML=`<small>دانش ترکیبی</small><b>${esc(obj.title)}</b><p class="k9913map-detail-body" style="direction:rtl;text-align:justify;text-align-last:right;max-height:260px;overflow-y:auto;overflow-x:hidden;padding-inline-end:7px;line-height:2">${esc(obj.synthesis||'')}</p><div>${(obj.conceptTitles||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div>`;
    }else{
      const related=rels.filter(r=>r.sourceRef===obj.id||r.targetRef===obj.id);
      box.innerHTML=`<small>مفهوم تثبیت‌شده · ${esc(obj.topic||'')}</small><b>${esc(obj.title)}</b><p class="k9913map-detail-body" style="direction:rtl;text-align:justify;text-align-last:right;max-height:220px;overflow-y:auto;overflow-x:hidden;padding-inline-end:7px;line-height:2">${esc(obj.definition||'')}</p><div><span class="k9913metric-centrality" style="background:#e9f4ff;color:#21679f;border:1px solid #c9e2f7">مرکزیت دانشی: ${fa(Math.round((obj.knowledgeCentrality||0)*100))}٪</span><span class="k9913metric-priority" style="background:#fff4dd;color:#9a6814;border:1px solid #efd9a7">اولویت شناختی: ${fa(Math.round((obj.cognitivePriority||0)*100))}٪</span><span class="k9913metric-relations" style="background:#f2eafb;color:#74479b;border:1px solid #dbc7ee">${fa(related.length)} رابطه دانشی</span></div>`;
    }
  });

  b.querySelectorAll('[data-k9929-ai-knowledge]').forEach(btn=>btn.onclick=async()=>{
    const k=knowledge.find(x=>x.id===btn.dataset.k9929AiKnowledge);if(!k)return;
    btn.disabled=true;const old=btn.textContent;btn.textContent='در حال تولید…';
    try{
      const r=await api('/api/v1/knowledge/macro-knowledge',{method:'POST',body:JSON.stringify({action:'ai_synthesize',candidate:k})});
      const s=r.suggestion||{};
      const implications=(s.implications||[]).length?`\n\nپیامدهای استنباطی:\n- ${(s.implications||[]).join('\n- ')}`:'';
      const assumptions=(s.assumptions||[]).length?`\n\nفرض‌ها/احتیاط‌ها:\n- ${(s.assumptions||[]).join('\n- ')}`:'';
      const edited=await SinaDialog.prompt('پیشنهاد هوش مصنوعی را بررسی و در صورت نیاز اصلاح کنید:',{title:s.title||'پیشنهاد دانش ترکیبی',value:(s.synthesis||k.synthesis||'')+implications+assumptions,confirmText:'استفاده از این متن'});
      if(edited!==null){k.synthesis=edited;const p=btn.closest('.k9913knowledge-card')?.querySelector('p');if(p)p.textContent=edited}
    }catch(e){await SinaDialog.alert(e.message,{title:'تولید دانش با هوش مصنوعی'})}
