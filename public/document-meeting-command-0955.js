(()=>{
window.__DOCUMENT_MEETING_COMMAND_BUILD__='0.9.5.5';
...
function k954CreateCalendarField(original,label,container=null){
 ...
 function render(){
   ...
   pop.innerHTML=`<div class="k954calhead"><button type="button" data-k954-prev>‹</button><b>${K954_MONTHS[jm-1]} ${k953EnToFa(jy)}</b><button type="button" data-k954-next>›</button></div><div class="k954week">${K954_WEEK.map(x=>`<span>${x}</span>`).join('')}</div><div class="k954days">${cells}</div><div class="k954calfoot"><button type="button" data-k954-today>امروز</button><button type="button" data-k954-clear>پاک کردن</button></div>`;
   pop.querySelector('[data-k954-prev]').onclick=()=>{cur.jm--;if(cur.jm<1){cur.jm=12;cur.jy--}render()};
   pop.querySelector('[data-k954-next]').onclick=()=>{cur.jm++;if(cur.jm>12){cur.jm=1;cur.jy++}render()};
...
function k955StructureHtml(structure){
 ...
 return structure.blocks.map(b=>{
  if(b.type==='paragraph')return `<p class="k955docp">${k955Esc(b.text)}</p>`;
  if(b.type==='table')return `<div class="k955tablewrap"><table class="k955doctable"><tbody>${(b.rows||[]).map(row=>`<tr>${row.map(c=>`<td>${k955Esc(c.text||'').replace(/\n/g,'<br>')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
  return'';
 }).join('');
}
async function k955RenderStructuredDoc(){
 ...
 }catch{full.dataset.k955Structured='0'}
}
...
function enhance(){document.querySelectorAll('#k76form').forEach(f=>{enhanceUploadForm(f);k953EnhanceMetadata(f);k955ComposeForm(f)});const modal=document.getElementById('k91docmodal');if(modal){collapseRelations(modal.querySelector('.k944relations'));addPopupTools(modal);k955RenderStructuredDoc()}}
