(()=>{
window.__DOCUMENT_MEETING_COMMAND_BUILD__='0.9.9.0.31';
...
function k9919DisplayDate(v){
 if(!v)return'';
 const raw=String(v||'').trim();
 const j=k953IsoToJalali(raw);
 if(j)return j;
 try{
   const d=new Date(raw);
   if(!Number.isNaN(d.getTime()))return new Intl.DateTimeFormat('fa-IR-u-ca-persian',{year:'numeric',month:'2-digit',day:'2-digit'}).format(d);
 }catch{}
 return toFa(raw);
}
function renderDocCommand(d){
 ...
 const filterSummary=d.parsed?.issuer?`<div class="k9920parsed"><span>مرجع تشخیص‌داده‌شده: <b>${esc(d.parsed.issuer)}</b></span>${d.parsed.meetingNumber?`<span>جلسه: <b>${esc(toFa(d.parsed.meetingNumber))}</b></span>`:''}</div>`:'';
