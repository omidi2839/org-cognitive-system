(()=>{
window.__KNOWLEDGE_GOVERNANCE_BUILD__='0.9.9.2.12';
...
async function counters(force=false){
 ...
 // Cache is only a rendering optimization. Forced mutation refresh bypasses it.
 if(!force&&k9931CounterCache&&(now-k9931CounterFetchedAt)<10000){
 ...
 try{
  ...
  b.querySelectorAll('[data-k9913-node]').forEach(node=>node.onclick=()=>{
    ...
    if(obj.conceptRefs){
      ...
      box.innerHTML=`<small>مفهوم تثبیت‌شده · ${esc(obj.topic||'')}</small><b>${esc(obj.title)}</b><p>${esc(obj.definition||'')}</p><div><span class="k9913metric-centrality">مرکزیت دانشی: ${fa(Math.round((obj.knowledgeCentrality||0)*100))}٪</span><span class="k9913metric-priority">اولویت شناختی: ${fa(Math.round((obj.cognitivePriority||0)*100))}٪</span><span class="k9913metric-relations">${fa(related.length)} رابطه دانشی</span></div>`;
    }
  });
