(()=>{
window.__DOCUMENT_BANK_BUILD__='0.9.9.2.11';
const FA='۰۱۲۳۴۵۶۷۸۹';
...
async function k982LoadEditPermission(){
 k982CanEditDocuments=false;k982CanDeleteDocuments=false;
 try{
  ...
  const role=String(g.permissions?.role||'').toLowerCase();
  if(['system_admin','admin','administrator','manager','knowledge_admin'].includes(role))k982CanDeleteDocuments=true;
 }catch{}
 if(!k982CanDeleteDocuments){
  try{
   const r=await fetch('/api/v1/auth/session',{headers:{'content-type':'application/json','x-org-id':ORG}});
   const s=await r.json().catch(()=>({}));
   const role=String(s?.user?.role||'').toLowerCase();
   if(s?.authenticated&&['system_admin','admin','administrator','manager','knowledge_admin'].includes(role)){
    k982CanEditDocuments=true;k982CanDeleteDocuments=true;
   }
  }catch{}
 }
}
async function k982DeleteDocument(id,title=''){
 ...
 if(!ok)return;
 const result=await api('/api/v1/knowledge/document-governance?documentId='+encodeURIComponent(id),{method:'DELETE'});
 if(!result?.ok)throw Error(result?.message||'حذف سند نهایی نشد.');
 // Optimistic UI removal after a confirmed server-side delete.
 document.querySelectorAll(`[data-doc-id="${CSS.escape(String(id))}"],[data-document-id="${CSS.escape(String(id))}"]`).forEach(x=>x.remove());
 closeDocumentModal();
 window.dispatchEvent(new CustomEvent('sina:data-changed',{detail:{type:'document_deleted',documentId:id}}));
 await window.__SINA_REFRESH_COUNTERS__?.();
 await runBankSearch();
}
