(()=>{
window.__DOCUMENT_MEETING_COMMAND_BUILD__='0.9.5.6';
...
function k956InlineHtml(block){
 if(Array.isArray(block.inlineParts)&&block.inlineParts.length){
   return block.inlineParts.map(p=>{
     if(p.type==='fraction')return `<span class="k956fraction"><span>${k955Esc(k956FaDigits(p.numerator))}</span><span>${k955Esc(k956FaDigits(p.denominator))}</span></span>`;
     return k955Esc(k956FaDigits(p.text||''));
   }).join('');
 }
 return k955Esc(k956FaDigits(block.text||''));
}
function k955StructureHtml(structure){
 ...
 return structure.blocks.map(b=>{
  ...
  if(b.type==='table')return `<div class="k955tablewrap"><table class="k955doctable"><tbody>${(b.rows||[]).map(row=>`<tr>${row.map(c=>`<td class="${k956AlignClass(c.alignment).trim()}">${k955Esc(k956FaDigits(c.text||'')).replace(/\n/g,'<br>')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
  return'';
 }).join('');
}
...
async function k955RenderStructuredDoc(){
 ...
 }catch{ k956NormalizeDisplayedDigits(full); full.dataset.k955Structured='0'}
}
