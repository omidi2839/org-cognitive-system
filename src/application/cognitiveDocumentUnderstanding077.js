...
function questions(text,concepts,relations,units){
 const qs=[],push=(question,reason,target,evidence,ct,ctx={})=>qs.push({id:newId('CQ'),question,reason,target,evidence,claimType:ct,status:'open',semanticContext:ctx});
 for(const f of frames(text)){const aa=f.actions.map(x=>x.label),ee=f.entities.map(x=>x.label);
 if(aa.length>1)push(`در این گزاره، «${aa.join('، ')}» چه تفاوت مفهومی و کارکردی دارند؛ مراحل یک فرایندند یا کارکردهای مستقل؟`,'مرز و نسبت چند کنش کلیدی باید روشن شود.',aa.join('، '),f.text,f.claimType,{highlightConcepts:aa,interpretation:`سامانه ${aa.length} کارکرد مرتبط را در یک گزاره تشخیص داده، اما نوع رابطه آنها نیازمند احراز انسانی است.`,proposedRelations:['هم‌عرض','ترتیبی','مکمل یکدیگر','علّی/اثرگذار بر یکدیگر','جزء و کل','رابطه دیگری دارند'],expectedClarificationType:'relation_and_definition'});
 for(const x of aa.slice(0,4))push(`در همین گزاره، منظور سازمان از «${x}» دقیقاً چیست و تحقق آن چگونه قابل تشخیص است؟`,'تعریف سازمانی باید در بافت همان گزاره تکمیل شود.',x,f.text,f.claimType,{highlightConcepts:[x],interpretation:`«${x}» به‌عنوان کنش/کارکرد کلیدی این گزاره تشخیص داده شده است.`,proposedRelations:[],expectedClarificationType:'organizational_definition'});
 ...
 if(qs.length>=12)break}return qs.slice(0,12);
}
